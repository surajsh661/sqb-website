...
/**
 * Manifesto headline: CINEMA × AI.
 * On scroll-in, CINEMA splits in the middle and a cream cinema camera pops into
 * the gap (record dot blinks, lens rays flash); at the same time AI splits and
 * three yellow sparks twinkle in its gap. Both hold ~2.5s then close back to
 * "CINEMA × AI". Re-triggers on each entry via IntersectionObserver.
 */
export default function ManifestoHeadline() {
  ...
  useEffect(() => {
    ...
    const onEnd = (e: AnimationEvent) => {
      if (e.animationName === 'mhCamGap') { el.classList.remove('play'); armed = true; }
    };
    ...
    io.observe(el);
    return () => { io.disconnect(); el.removeEventListener('animationend', onEnd); };
  }, []);
  ...
  return (
    <h2 className="mh mh-shake" ref={ref}>
      <span className="mh-nowrap mh-word">
        <span>CIN</span>{camera}<span>EMA</span>
      </span>
      <span className="mh-x" aria-hidden="true">×</span>
      <span className="mh-nowrap mh-word mh-word-ai">
        <span>A</span>{stars}<span>I</span>
      </span>
      <span className="sr-only">Cinema and AI.</span>
    </h2>
  );
}
